...
struct power_supply mmm;
struct power_supply* aaa=&mmm;
//struct delayed_work mmm1;
//struct timer_list mmm2;

//struct bdata* ccc=kmalloc(sizeof(struct bdata),__GFP_ZERO|GFP_KERNEL);


        pr_err("fuck %08x  %08x %08x",sizeof(spinlock_t),aaa,&aaa->changed_lock);
	if (ret == sizeof(mmm)) return -1;
